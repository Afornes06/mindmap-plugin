...
async function genereOrganigramme(table) {
  ...
  chart = new d3.OrgChart()
    ...
    .nodeContent(function (d, i, arr, state) {
      ...
      // Measure the text to adjust the height of the description box
      const textHeight = 40; // Example fixed height for text, adjust as needed
      const totalHeight = d.height + textHeight;

      return `
        <div style='width:${d.width}px;height:${totalHeight}px;padding-top:${imageDiffVert - 2}px;padding-left:1px;padding-right:1px'>
          <div style="font-family: 'Inter', sans-serif;background-color:${color}; margin-left:-1px;width:${d.width - 2}px;height:${textHeight}px;border-radius:10px;border: 1px solid #E4E2E9">
          ...
          <div style="background-color:${color};margin-top:10px;margin-left:${15}px;border-radius:100px;width:50px;height:50px;" ></div>
          <div style="margin-top:${-imageDiffVert - 20}px;">
            <img src="${d.data[chpImage] && d.data[chpImage][0] ? getImage(d.data[chpImage][0]) : icone}" style="margin-left:${20}px;border-radius:100px;width:40px;height:40px;" />
          </div>
          <div style="margin-top:10px;padding:10px;background-color:${color};border-radius:10px;border: 1px solid #E4E2E9;">
            <!-- Description box -->
            <div style="font-size:12px;color:#08011E;">Description goes here...</div>
          </div>
        </div>
      `;
    })
...
async function genereOrganigramme(table) {
  ...
  chart = new d3.OrgChart()
    ...
    .nodeContent(function (d, i, arr, state) {
      ...
      const text = `${d.data[chpNom]}<br>${d.data[chpFonction] || ""}<br>${d.data[chpDirection] || ""}`;
      const textHeight = measureTextHeight(text, '15px', 'Inter, sans-serif');

      // Adjust the total height
      const totalHeight = d.height + textHeight + 60; // Adding some padding
